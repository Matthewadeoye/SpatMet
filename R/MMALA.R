...
#Riemann Manifold Langevin updates -- Sampling
FRANK_FFBS_INFERENCE<- function(y, e_it, Modeltype, adjmat, step_sizes, num_iteration = 15000, sdBs=0.03, sdGs=0.05, sdCop=0.7){
  start_time <- Sys.time()
  ndept <- nrow(e_it)
  time <- ncol(e_it)
  nstrain<- dim(y)[3]
  ...
  Model<- ifelse(Modeltype>0,1,0)
  n_copParams<- ifelse(Modeltype %in% c(0,1,2,5),0,1)
  ...
  #Compute gradients
  if(Modeltype %in% c(1,2)){
    JointTPM<- Multipurpose_JointTransitionMatrix2(MC_chain[1,1:num_Gammas], nstrain, MC_chain[1,num_Gammas+3+time+12+ndept+nstrain+nstrain], Modeltype)
  }else if(Modeltype %in% c(3,4)){
    JointTPM<- Multipurpose_JointTransitionMatrix2(MC_chain[1,1:num_Gammas], nstrain, MC_chain[1,num_Gammas+3+time+12+ndept+nstrain+nstrain+(1:n_copParams)], Modeltype)
    JointTPM<- ifelse(JointTPM<=0,1e-6,JointTPM)
    JointTPM<- ifelse(JointTPM>=1,1-1e-6,JointTPM)
    if(any(!is.finite(JointTPM))) JointTPM<- initGs
  }else if(Modeltype == 5){
  ...
  for (i in 2:num_iteration) {
    ...
    if(Model == 0){
      ...
      if(Modeltype %in% c(1,2)){
        ...
        trans_Psi_current <- asinh(MC_chain[i-1, ncol(MC_chain)])
        trans_Psi_proposed <- rnorm(1, mean = trans_Psi_current, sd = sdCop)
        Psi_proposed <- sinh(trans_Psi_proposed)

        proposalcurrentcop<- dnorm(trans_Psi_current, mean = trans_Psi_proposed, sd = sdCop, log = TRUE)
        proposalproposedcop<- dnorm(trans_Psi_proposed, mean = trans_Psi_current, sd = sdCop, log = TRUE)

        logJacobiancur<- log(cosh(trans_Psi_current))
        logJacobianprop<- log(cosh(trans_Psi_proposed))

        JointTPM1<- Multipurpose_JointTransitionMatrix_cpp2(proposedGs, nstrain, Psi_proposed, Modeltype)
        JointTPM1<- ifelse(JointTPM1<=0,1e-6,JointTPM1)
        JointTPM1<- ifelse(JointTPM1>=1,1-1e-6,JointTPM1)
        if(any(!is.finite(JointTPM1))){
          MC_chain[i, ncol(MC_chain)]<- MC_chain[i-1, ncol(MC_chain)]
          ...
          mh.ratio<- exp(likelihoodproposed + priorproposedGs + proposalcurrentcop + logJacobianprop
                         - likelihoodcurrent - priorcurrentGs - proposalproposedcop - logJacobiancur)
          ...
          if(!is.na(mh.ratio) && runif(1) < mh.ratio){
            ...
            grad_current<- grad_proposed
            JointTPM<- JointTPM1
            MC_chain[i, ncol(MC_chain)]<- Psi_proposed
          }
          else{
            MC_chain[i, 1:num_Gammas]<- MC_chain[i-1,1:num_Gammas]
            MC_chain[i, ncol(MC_chain)]<- MC_chain[i-1, ncol(MC_chain)]
...
log_lkj <- function(R, eta) {
  (eta - 1) * determinant(R, logarithm = TRUE)$modulus
}
...
gaussian_copula_grad_fisher<- function(u, copulaParams, nstrain, eta) {
  ...
  gdata::lowerTriangle(Sigma, byrow=FALSE) <- copulaParams

  if(length(u)==2){
    u<- rep(u[1],nstrain)
  }else{
    indc<- seq(1,2*nstrain-1,2)
    u<- u[indc]
  }
  x <- qnorm(u)
  ...
  for (i in 2:num_iteration) {
    ...
    if(Model == 0){
      ...
      if(Modeltype %in% c(1,2)){
        ...
        eps_cop <- rnorm(n_copParams)
        proposedcops <-  as.numeric(MC_chain[i-1, num_Gammas+3+time+12+ndept+nstrain+nstrain+(1:n_copParams)] + 0.5 * step_sizes$cop^2 * currentGradcop$gradients + step_sizes$cop * eps_cop)
        ...
        JointTPM1<- Multipurpose_JointTransitionMatrix(proposedGs, nstrain, proposedcops, Modeltype)
        ...
        if(any(!is.finite(JointTPM1))){
          ...
          mh.ratioGC<- exp(likelihoodproposed + priorproposedGs + priorproposedCops + q_currcop
                           - likelihoodcurrent - priorcurrentGs - priorcurrentCops - q_propcop)

          #print(mh.ratioGC)

          if(!is.na(mh.ratioGC) && runif(1) < mh.ratioGC){
            MC_chain[i, 1:num_Gammas]<- proposedGs
            MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+nstrain+(1:n_copParams)]<- proposedcops
            likelihoodcurrent<- likelihoodproposed
            grad_current<- grad_proposed
            currentGradcop<- proposedGradcop
            priorcurrentCops<- priorproposedCops
            JointTPM<- JointTPM1
          }
          else{
            MC_chain[i, 1:num_Gammas]<- MC_chain[i-1,1:num_Gammas]
